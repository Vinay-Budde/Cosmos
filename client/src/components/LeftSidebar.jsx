...
import { Search, Bell, Clock, Calendar, Video, Hash, ChevronDown, Settings, LayoutGrid } from 'lucide-react';
import { showToast } from '../utils/toastEmitter';

export default function LeftSidebar({ myUser, otherUsers, handRaisedBy = new Set() }) {
  const handleFeatureSoon = (name) => showToast(`${name} feature coming soon!`);

  return (
    <div className="w-[260px] h-full bg-[#fbfbfb] border-r border-slate-100 flex flex-col shrink-0 overflow-y-auto text-slate-800 z-[100] custom-scrollbar">
        {/* Search Bar */}
        <div className="p-4">
          <div className="w-full bg-white border border-slate-200/60 flex items-center px-3 py-1.5 rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.02)] hover:border-slate-300 transition-colors group">
            ...
            />
            <div className="bg-slate-50 px-1.5 py-0.5 rounded-lg border border-slate-200 flex items-center gap-0.5 shadow-sm">
